...
// ── Aviso de transferencia pendiente demasiado tiempo (24h / 42h / 48h) ──
...
function rAdmTabVehs(plazas){
  ...
  return`
    ${dups.length?`<div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:9px;padding:11px 14px;margin-bottom:14px">
      ...
      ${dups.map(d=>`<div style="font-size:11px;color:#7F1D1D;margin-bottom:3px">ECO <strong>${d.eco}</strong>: ${d.vehiculos.map(v=>`${v.unidad||'—'} (${flNombrePorCorreo(v.responsable)||'sin responsable'})`).join('  ·  ')}</div>`).join('')}
    ...
    ${errores.length?errores.slice(0,30).map(e=>`
      <div style="border:1px solid #FECACA;background:#FEF2F2;border-radius:10px;padding:10px 14px;margin-bottom:8px">
        <div style="display:flex;justify-content:space-between;gap:8px;margin-bottom:4px">
          <strong style="font-size:12px;color:#991B1B">${e.nombre||flNombrePorCorreo(e.email)||'—'}</strong>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:10px;align-items:end;max-width:780px">
      ...
      <div>
        ...
        <select id="adm-ur-eco" style="width:100%;padding:9px 12px;border:1.5px solid #E2E8F0;border-radius:9px;font-family:inherit;font-size:13px;outline:none">
          ...
          ${flV.map(v=>`<option value="${v.eco}">ECO ${v.eco} — ${v.unidad||''} ${v.responsable&&v.responsable!=='—'?'('+flNombrePorCorreo(v.responsable)+')':'' }</option>`).join('')}
        </select>
      </div>
      <div>
      ...
      ${atencion.map(({v,motivos})=>{
        ...
        return`<div onclick="flSbSel('${v.id}')" style="display:flex;align-items:center;gap:14px;padding:14px 16px;border:1px solid #E8EDF5;border-radius:12px;cursor:pointer;transition:box-shadow .15s" onmouseover="this.style.boxShadow='0 2px 10px rgba(0,0,0,.06)'" onmouseout="this.style.boxShadow='none'">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:12px;color:#64748B;margin-top:2px">${v.responsable&&v.responsable!=='—'?flNombrePorCorreo(v.responsable):'Sin responsable asignado'}</div>
          </div>
      ...
      ${listaOrd.length?listaOrd.map(x=>{
        ...
        return`<div style="display:flex;gap:14px;padding:16px 18px;border-radius:14px;${esActivo?'background:#FEF2F2;border:1px solid #FECACA':'border:1px solid #E8EDF5'}">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:11px;${esActivo?'color:#B91C1C':'color:#94A3B8'};margin-top:6px">Reportado por ${flNombrePorCorreo(x.reportadoPor)||'—'} · ${dt(x.creadoEn)}${x.resueltoEn?` · Resuelto ${dt(x.resueltoEn)}`:''}</div>
          </div>
  ...
  rp.innerHTML=`
    ${comAct?`<div style="background:#FEF2F2;border:1px solid #FCA5A5;border-radius:9px;padding:9px 12px;margin:12px 20px 0;display:flex;align-items:flex-start;gap:8px">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10.5px;color:#7F1D1D;margin-top:2px;overflow-wrap:anywhere"><strong>Responsable:</strong> ${flNombrePorCorreo(comAct.responsable)||'—'}</div>
    ...
    ${card('Información general',`
      ${fila('Responsable',v.responsable&&v.responsable!=='—'?flNombrePorCorreo(v.responsable):'<span style="color:#B91C1C">Sin asignar</span>')}
...
function flRPHistBody(s){
  ...
  return`
    ...
    <div style="font-size:11.5px;font-weight:600;color:#0A1628;margin-bottom:8px">${flNombrePorCorreo(s.solicitante||s.creadoPor)||'—'}</div>
...
function flPanorCardHTML(v){
  ...
  return`<div class="fl-panor-card ${sel?'sel':''}" onclick="flPanorToggle('${v.id}')" title="ECO ${v.eco} · ${v.unidad||''}">
    ...
    <div class="fl-panor-info">
      ...
      <div class="fl-panor-resp">${I.user} ${flNombrePorCorreo(v.responsable)||'Sin responsable'}</div>
    </div>
  </div>`;
}
...
function flPanorResumenVeh(v){
  ...
  return`<div class="fl-panor-sum-card"style="padding: 15px 20px; box-sizing: border-box; display: flex; flex-direction: column; width: 100%;">
    ...
    <div class="fl-panor-sum-sec">
      ...
      ${ultReporte?`<div style="font-size:11.5px;color:#374151"><strong>${ultReporte.tipo||'—'}</strong> · ${flNombrePorCorreo(ultReporte.solicitante||ultReporte.creadoPor)||'—'} · ${dt(ultReporte.creadoEn)} ${hBadge(ultReporte.estatus)}</div>`
...
function tSols(list,pA){
  ...
  return`<div class="fl-tw"><table class="fl-t"><thead><tr>
    ...
    <td style="font-size:11px">${flNombrePorCorreo(s.solicitante)||'—'}</td>
  ...
  ov.innerHTML=`<div class="fl-modal" style="max-width:520px">
    ...
    <div style="padding:16px 20px">
      ...
      <div style="display:flex;align-items:center;gap:10px;background:#F8FAFC;border-radius:10px;padding:10px 12px;margin-bottom:12px">
        <div style="flex:1"><div style="font-size:9.5px;color:#94A3B8;font-weight:800;text-transform:uppercase">Entrega</div><div style="font-size:13px;font-weight:700">${flNombrePorCorreo(t.entregaNombre)||'—'}</div></div>
...
function rResumenChkSemTabla(datos){
  ...
  return`<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse">
    <thead><tr>
      ...
      <td style="${td}">${flNombrePorCorreo(d.responsable)}</td>
      ...
      <td style="${td}">${flNombrePorCorreo(d.tecnico)}</td>
...
window.flResumenChkSemPDF=function(semSel){
  ...
  const filas=datos.map(d=>`<tr style="border-bottom:1px solid #F1F5F9;${d.hizoChecklist?'':'background:#FFFBEB'}">
    ...
    <td style="padding:6px;font-size:10.5px">${flNombrePorCorreo(d.responsable)}</td>
    ...
    <td style="padding:6px;font-size:10.5px">${flNombrePorCorreo(d.tecnico)}</td>
...
window.flReporteUsoPDF=function(){
  ...
  const filas=datos.map(t=>`
    <tr style="border-bottom:2px solid #0A1628"><td colspan="4" style="padding:10px 6px 4px;font-weight:900;font-size:12.5px">${t.nombre}</td></tr>
...
window.flGenerarPDFChkSem = async function(id) {
  ...
  <div class="grid3">
    ...
    <div class="field"><label>Responsable</label><span>${flNombrePorCorreo(v.responsable)||'—'}</span></div>
  ...
  <table>
    ...
    <tbody>
      ${flS.filter(s=>!['Cerrada','Rechazada'].includes(s.estatus)).slice(0,20).map(s=>{
        ...
        return`<tr><td>${s.tipo||'—'}</td><td style="font-family:monospace;font-weight:700">ECO ${s.vehiculoEco||'—'}</td><td>${(s.vehiculo||'—').split('·')[1]?.trim()||s.vehiculo||'—'}</td><td>${flNombrePorCorreo(s.solicitante||s.creadoPor)||'—'}</td><td><span class="badge" style="background:${cl}22;color:${cl}">${s.estatus}</span></td><td>${s.creadoEn?s.creadoEn.substring(0,10):'—'}</td></tr>`;
      }).join('')}
    </tbody>
  </table>
  ...
  setContent(padded(`
    ...
    `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:10px">
      ${bajas.map(v=>`<div style="background:#fff;border:1px solid #E8EDF5;border-radius:11px;padding:12px 14px;display:flex;gap:10px;align-items:center;cursor:pointer;transition:border-color .15s" onmouseenter="this.style.borderColor='#FCA5A5'" onmouseleave="this.style.borderColor='#E8EDF5'" onclick="flVerVeh('${v.id}')">
        ...
        <div style="flex:1;min-width:0"><div style="font-size:11.5px;font-weight:800;font-family:'JetBrains Mono',monospace">ECO ${v.eco}</div><div style="font-size:11.5px;font-weight:600;color:#374151">${v.unidad||'—'} ${v.año||''}</div><div style="font-size:10px;color:#94A3B8">${flNombrePorCorreo(v.responsable)||'—'}</div></div>
  ...
  const filas = evs.map(e => {
    ...
    return `<div onclick="flVerSol('${e.id}')" style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:#F8FAFC;border-radius:9px;border:1px solid #E2E8F0;cursor:pointer;transition:.12s" onmouseover="this.style.background='#EFF6FF'" onmouseout="this.style.background='#F8FAFC'">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:10px;color:#94A3B8;margin-top:1px">${flNombrePorCorreo(e.solicitante||e.creadoPor)||'—'}${e.montoCotizacion?` · $${Number(e.montoCotizacion).toLocaleString('es-MX')}`:''}</div>
      </div>
      ...
      <div class="fl-modal" style="max-width:640px;width:100%;max-height:90vh;overflow-y:auto">
        ...
        <div class="fl-mb" style="display:flex;flex-direction:column;gap:14px">
          ...
          <div style="background:#F8FAFC;border-radius:10px;padding:12px 14px;font-size:11px">
            ...
            <span style="color:#94A3B8">Prioridad: ${s.prior||'Normal'} · Solicitante: ${flNombrePorCorreo(s.solicitante||s.creadoPor)||'—'}</span>
          </div>
        ...
        <div style="background:linear-gradient(135deg,#0A1628,#1E3A5F);padding:20px 24px;flex-shrink:0">
          <div style="display:flex;align-items:center;justify-content:space-between">
            <div>
              ...
              <div style="font-size:12px;color:rgba(255,255,255,.6);margin-top:3px">${v?v.unidad+' '+v.año:''} · ${flNombrePorCorreo(s.solicitante||s.creadoPor)||'—'}</div>
            </div>
            <div style="text-align:right">
        ...
        <div style="overflow-y:auto;flex:1;padding:20px 24px">
          ...
          ${seccion('📋 Solicitud original', `
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0">
              ...
              ${campo('Solicitante', flNombrePorCorreo(s.solicitante||s.creadoPor))}
          ...
          ${seccion('⏱ Línea de tiempo', `
            <div style="display:flex;flex-direction:column;gap:6px">
              ${[
                ['Solicitud creada', fmt(s.creadoEn), flNombrePorCorreo(s.solicitante||s.creadoPor), '#6D28D9'],
  ...
  ov3.innerHTML=`<div class="fl-modal" style="max-width:500px">
    <div class="fl-mh">
      <div>
        ...
        <div style="font-size:10px;color:#64748B;margin-top:2px">ECO ${t.vehiculoEco||'—'} · Asignado: ${t.asignadoNombre||flNombrePorCorreo(t.asignadoA)||'—'}</div>
      </div>
      <button class="fl-mx" onclick="this.closest('.fl-ov').remove()">✕</button>
    </div>
    <div class="fl-mb" style="display:flex;flex-direction:column;gap:10px">
...
function _renderTareasPanel() {
  ...
  const tecnicos = [...new Map(_flTareasAll.map(t=>[t.asignadoA,{email:t.asignadoA,nombre:t.asignadoNombre||flNombrePorCorreo(t.asignadoA)}])).values()];
        ...
        ${lista.map((t,idx)=>{
          ...
          return `<tr style="border-bottom:1px solid #F1F5F9;background:${idx%2===0?'#fff':'#FAFBFD'};${venc?'border-left:3px solid #EF4444':''}" onmouseover="this.style.background='#F0F4FF'" onmouseout="this.style.background='${idx%2===0?'#fff':'#FAFBFD'}'">
            ...
            <td style="padding:10px 12px;white-space:nowrap">
              <div style="font-weight:600">${t.asignadoNombre||flNombrePorCorreo(t.asignadoA)||'—'}</div>
            </td>
